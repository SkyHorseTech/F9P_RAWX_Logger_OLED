  ...
  switch(loop_step) {
        ...
        if (base_mode == true) {
          if (digitalRead(SurveyInPin) == LOW) {
           ...
           // delay(1000); //custom Sky Horse
            ...
            //display.setTextSize(1);      // Normal 1:1 pixel scale
            display.setCursor(2, 30);
            display.write("SURVEY_IN mode selected"); //custom Sky Horse
            display.display(); 
    ...
    case open_file: {
      ...
      if (rawx_dataFile.open(rawx_filename, O_CREAT | O_WRITE | O_EXCL)) {
        ...
        delay(1100); //custom Sky Horse

            display.clearDisplay(); // Clear display buffer
            //display.setTextColor(WHITE);
            display.setTextSize(2);      // Normal 1:1 pixel scale
            display.setCursor(2, 1);
            display.write("Logging to "); //custom Sky Horse
            //display.display();
            //display.setCursor(4, 4);
            display.write(rawx_filename); //custom Sky Horse
            display.display(); 
      ...
      // if the file isn't open, pop up an error:
        ...
        // delay(1000); //custom Sky Horse
            ...
            display.setTextSize(1);      // Normal 1:1 pixel scale
            display.setCursor(2, 1);
            display.write("Error opening RAWX file!"); //custom Sky Horse
            display.display();
            display.setCursor(3, 30);
            display.write("Waiting for reset..."); //custom Sky Horse
            display.display(); 
      ...
      if (!rawx_dataFile.timestamp(T_CREATE, (RTCyear+2000), RTCmonth, RTCday, RTChours, RTCminutes, RTCseconds)) {
        ...
        // delay(1000); //custom Sky Horse
            ...
            display.setTextSize(1);      // Normal 1:1 pixel scale
            display.setCursor(2, 1);
            display.write("Could not create timestamp!"); //custom Sky Horse
            display.display(); 
      }
#else
      ...
      if (bufAvail > 0) {
        ...
        if (bufAvail > maxSerialBufferAvailable) {
          ...
          Serial.println(maxSerialBufferAvailable);
            ...
            display.setTextSize(1);      // Normal 1:1 pixel scale
            display.setCursor(2, 1);
            display.write("Max bufAvail: "); //custom Sky Horse
            display.setCursor(3, 3);
            display.write(maxSerialBufferAvailable); //custom Sky Horse
            display.display(); 
      }
        }
#endif  
        ...
        if (bufferPointer == SDpacket) {
          ...
          if (numBytes != SDpacket) {
            ...
            display.setTextSize(1);      // Normal 1:1 pixel scale
            display.setCursor(2, 1);
            display.write("SD write error! Write size was "); //custom Sky Horse
            display.setCursor(3, 3);
            display.write(SDpacket); //custom Sky Horse
            display.setCursor(4, 4);
            display.write("Number of bytes written: "); //custom Sky Horse
            display.setCursor(5, 5);
            display.write(numBytes); //custom Sky Horse
            display.display(); 
        ...
        switch (ubx_nmea_state) {
          case (looking_for_B5_dollar): {
            if (c == 0xB5) { // Have we found Sync Char 1 (0xB5) if we were expecting one?
              ...
              display.setTextSize(1);      // Normal 1:1 pixel scale
              display.setCursor(2, 1);
              display.write("Was expecting Sync Char 0xB5 or "); //custom Sky Horse
              display.setCursor(3, 3);
              display.write("an NMEA $ but did not receive one!"); //custom Sky Horse
              display.display(); 
          ...
          case (looking_for_62): {
            if (c == 0x62) { // Have we found Sync Char 2 (0x62) when we were expecting one?
              ...
              display.setTextSize(1);      // Normal 1:1 pixel scale
              display.setCursor(2, 1);
              display.write("Was expecting Sync Char 0x62"); //custom Sky Horse
              display.setCursor(2, 2);
              display.write(" but did not receive one!"); //custom Sky Horse
              display.display(); 
          ...
          case (looking_for_class): {
            ...
            if ((ubx_class != 0x02) and (ubx_class != 0x0d) and (ubx_class != 0x01)) {
              ...
              display.setTextSize(1);      // Normal 1:1 pixel scale
              display.setCursor(2, 1);
              display.write("Was expecting Class of 0x02 or 0x0d"); //custom Sky Horse
              display.setCursor(2, 2);
              display.write(" but did not receive one!"); //custom Sky Horse
              display.display(); 
          ...
          case (looking_for_ID): {
            ...
            if ((ubx_class == 0x02) and ((ubx_ID != 0x15) and (ubx_ID != 0x13))) {
              ...
              display.setTextSize(1);      // Normal 1:1 pixel scale
              display.setCursor(2, 1);
              display.write("Was expecting ID of 0x15 or 0x13"); //custom Sky Horse
              display.setCursor(2, 2);
              display.write(" but did not receive one!"); //custom Sky Horse
              display.display(); 
              ...
              display.setTextSize(1);      // Normal 1:1 pixel scale
              display.setCursor(2, 1);
              display.write("Was expecting ID of 0x03 "); //custom Sky Horse
              display.setCursor(2, 2);
              display.write(" but did not receive one!"); //custom Sky Horse
              display.display(); 
              ...
              display.setTextSize(1);      // Normal 1:1 pixel scale
              display.setCursor(2, 1);
              display.write("Was expecting ID of 0x02 or 0x07"); //custom Sky Horse
              display.setCursor(2, 2);
              display.write("or 0x03 but did not receive one!"); //custom Sky Horse
              display.display(); 
          ...
          case (looking_for_checksum_B): {
            ...
            if ((ubx_expected_checksum_A != ubx_checksum_A) or (ubx_expected_checksum_B != ubx_checksum_B)) {
              ...
              display.setTextSize(1);      // Normal 1:1 pixel scale
              display.setCursor(2, 1);
              display.write("UBX checksum error!"); //custom Sky Horse
              display.display(); 
          ...
          case (looking_for_asterix): {
            ...
            if (ubx_length > max_nmea_len) { // If the length is greater than max_nmea_len, something bad must have happened (sync_lost)
              ...
              display.setTextSize(1);      // Normal 1:1 pixel scale
              display.setCursor(2, 1);
              display.write("Excessive NMEA message length!"); //custom Sky Horse
              display.display(); 
          ...
          case (looking_for_csum2): {
            ...
            if ((nmea_csum1 != nmea_expected_csum1) or (nmea_csum2 != nmea_expected_csum2)) {
              ...
              display.setTextSize(1);      // Normal 1:1 pixel scale
              display.setCursor(2, 1);
              display.write("NMEA checksum error!"); //custom Sky Horse
              display.display(); 
          ...
          case (looking_for_term1): {
            ...
            if (c != '\r') {
              ...
              display.setTextSize(1);      // Normal 1:1 pixel scale
              display.setCursor(2, 1);
              display.write("NMEA CR not found!"); //custom Sky Horse
              display.display(); 
          ...
          case (looking_for_term2): {
            ...
            if (c != '\n') {
              ...
              display.setTextSize(1);      // Normal 1:1 pixel scale
              display.setCursor(2, 1);
              display.write("NMEA LF not found!"); //custom Sky Horse
              display.display(); 
    ...
    case new_file: {
      ...
      Serial.println("File closed!");
              ...
              display.write("File"); //custom Sky Horse
              display.setCursor(10, 36);
              display.write("Closed!"); //custom Sky Horse
              display.display(); 
    ...
    case close_file: {
      ...
      if (stop_pressed == true) {
        ...
        delay(10000);
        //loop_step = start_rawx; // loop round again and restart rawx messages before opening a new file Custom SkyHorse
        loop_step = restart_file; // loop round again and restart rawx messages before opening a new file Custom SkyHorse
        stop_pressed = false; //custom Sky Horse

        display.clearDisplay(); // Clear display buffer
        break; //custom Sky Horse
        ...
        Serial.println("Battery must be low - waiting for it to recover...");
              ...
              display.write("Battery must be low"); //custom Sky Horse
              display.setCursor(2, 7);
              display.write("waiting for it to recover..."); //custom Sky Horse
              display.display(); 
    ...
    case restart_file: {
      ...
      if (!rawx_dataFile.timestamp(T_WRITE, (RTCyear+2000), RTCmonth, RTCday, RTChours, RTCminutes, RTCseconds)) {
        Serial.println("Warning! Could not set file write timestamp!");
              ...
              display.setTextSize(1);      // Normal 1:1 pixel scale
              display.setCursor(2, 5);
              display.write("Warning! Could not set file write timestamp!"); //custom Sky Horse
              display.display(); 
